...
namespace constexpr_algorithms_detail {
...
{
	::constexpr_algorithms_detail::consteval_for_T<size_t, First, Last>(f);
}
...
constexpr void constexpr_for_fold([[maybe_unused]] Functor&& f) noexcept
{
	if constexpr (First < Last)
	{
		...
		::constexpr_algorithms_detail::constexpr_for_fold_impl<First>(std::forward<Functor>(f), std::make_index_sequence<Last - First>{});
	}
}
