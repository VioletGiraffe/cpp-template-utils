...
| `container/chunked_deque.hpp` | `chunked_deque` over fixed-size blocks with a per-block occupancy bitmask: push/pop at both ends, erasure and insertion anywhere, and no relocation on erasure. A push extends a block's live range but never fills a gap inside it, so sequence order is exact and a block holds its allocation until it drains. `operator[]` is linear in the block count. `reserve()` pre-allocates blocks and retains them across drains; `shrink_to_fit()` gives them back. |
