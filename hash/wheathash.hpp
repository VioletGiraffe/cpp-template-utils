...
inline constexpr uint64_t _wheatmum(uint64_t A, uint64_t B) {
	uint64_t r = A * B;
	return r - (r >> 32);
}

inline uint64_t _wheatr08(const uint8_t *p){ uint8_t  v; memcpy(&v, p, 1); return v; }
inline uint64_t _wheatr16(const uint8_t *p){ uint16_t v; memcpy(&v, p, 2); return v; }
